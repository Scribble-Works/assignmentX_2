...
const { data: workbook1Strand } = await client
  .from("Workbook1")
  .select("strand_name")
  .eq("id", strand_ref_id)
  .maybeSingle();
const strandName = (workbook1Strand?.strand_name ?? "").trim();

const title = (substrand[0].title ?? "").trim();
const conceptNote = strands[0].concept_notes;
const bece = strands[0].BECE_Qquestions;

// Canonical completion key used by BOTH this list and the shared quiz
// component (components/SubstrandQuiz.vue stores the selected indicator as
// `substrand-<id>`). Keeping the keys identical is what makes a completed
// indicator recognized on return and lets the progress counter increment.
...
const solveProblem = () => {
  ...
  if (allQuizzesCompleted.value) {
    navigateTo(
      `/learning-modules/assignment_workbook1/strand-${strand_ref_id}/substrand-${substrand_ref}/quiz/problem-set-${substrand_ref_id}`,
    );
  }
};
            ...
            <v-row>
              ...
              <v-col class="mt-15">
                <p>
                  ...
                  real-world situations using maths. There might be more than
                  one way — so be bold, be creative, and show the wow!
                </p>
                <v-btn
                  @click="solveProblem"
                  class="mt-5"
