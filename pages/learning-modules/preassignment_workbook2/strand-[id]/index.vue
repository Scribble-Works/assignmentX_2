...
const { data: workbook } = await client.from('pre-assignment_Workbook2').select().eq('id', id);
const { data: strand1 } = await client.from('preassignment_workbook2_substrands_contents').select().eq('id', id);
const substrand_ref = strand1[0].substrand_ref;
const { data: substrands } = await client.from('preassignment_workbook2_strand_substrands_lists').select().eq('strand_ref', id)
const conceptNote = workbook[0].concept_notes;
const strandNumber = substrands[0].strand_ref;
const { data: strandtitle } = await client.from('pre-assignment_Workbook2').select().eq('id', strandNumber);
        ...
        <v-container>
            ...
            <div class="mt-10">
                <v-row class="mt-n5" v-for="substrand in substrands" :key="substrand.id">
                    <v-col>
                        <NuxtLink
                            :to="'/learning-modules/preassignment_workbook2/strand-' + substrand.strand_ref + '/substrand-' + substrand.route + '/'">
                            <v-card>
