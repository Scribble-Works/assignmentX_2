...
const quizTitle = computed(
  () => `Pre-Course Quiz - Substrand ${substrandRefId}`,
);
...
const handleMarkAsCompleted = () => {
  ...
  if (markAsCompleted.value) {
    ...
    console.log(
      `Pre-quiz marked as completed for substrand: ${substrandRefId}`,
    );
  } else {
    // Unmark pre-quiz as completed
    unmarkQuizCompleted(substrandQuizKey);
    console.log(
      `Pre-quiz unmarked as completed for substrand: ${substrandRefId}`,
    );
  }
...
const goToContent = () => {
  ...
  if (!isQuizCompleted(substrandQuizKey)) {
    ...
    console.log(
      `Pre-quiz automatically marked as completed for substrand: ${substrandRefId}`,
    );
  }
  ...
  router.push(
    `/learning-modules/preassignment_workbook2/strand-${strandId}/substrand-${substrandRoute}/${targetContentId}`,
  );
};
...
const goBackToSubstrand = () => {
  ...
  router.push(
    `/learning-modules/preassignment_workbook2/strand-${strandId}/substrand-${substrandRoute}`,
  );
};
...
const loadQuestions = async () => {
  ...
  try {
    ...
    console.log(
      `[Quiz] 🚀 Starting to load questions for substrand: ${substrandRefId}`,
    );
    ...
    if (strapiQuestions && strapiQuestions.length > 0) {
      ...
      console.log(
        `[Quiz] ✅ Successfully loaded ${strapiQuestions.length} questions from Strapi`,
      );
      ...
      console.error(
        `[Quiz] ❌ Strapi returned null - check Strapi connection and topic ID`,
      );
      questions.value = [];
    } else {
      ...
      console.error(
        `[Quiz] ❌ No questions found in Strapi for topic ID: ${topicId}`,
      );
      console.error(
        `[Quiz] Make sure questions are added to this topic in Strapi`,
      );
      questions.value = [];
    }
  } catch (error) {
    ...
    if (questions.value.length > 0) {
      answers.value = new Array(questions.value.length).fill(null);
      console.log(
        `[Quiz] ✅ Initialized quiz with ${questions.value.length} questions`,
      );
    } else {
      console.warn(
        `[Quiz] ⚠️ No questions available for substrand: ${substrandRefId}`,
      );
    }
  }
};
...
onMounted(() => {
  ...
  markAsCompleted.value =
    isQuizCompleted(substrandQuizKey) || isQuizCompleted(substrandQuizKeyStr);
});
</script>
