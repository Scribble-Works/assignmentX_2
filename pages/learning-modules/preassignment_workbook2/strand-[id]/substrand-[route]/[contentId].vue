...
const { data: substrands } = await client.from('preassignment_workbook2_strand_substrands_lists').select().eq('route', strand_ref);

const strand_ref_id = substrands[0].strand_ref;
const substrand_ref_id = substrands[0].id;
const { data: files } = await client.from('preassignment_workbook2_substrands_contents').select().eq('id', substrand_ref_id);

const { data: indicators_content } = await client.from('preassignment_workbook2_substrand_indicators').select().eq('id', courseContentId);
